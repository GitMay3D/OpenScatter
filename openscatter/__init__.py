...
class SNA_PT_OPENSCATTER_85C9A(bpy.types.Panel):
    bl_label = 'OpenScatter'
    bl_idname = 'SNA_PT_OPENSCATTER_85C9A'
    ...
    def draw(self, context):
        ...
        if (bpy.context.scene.sna_emitter_object == None):
            ...
            if (len(bpy.context.scene.sna_scattersystemcollection) > 0):
                ...
                layout_function = layout
                sna_func_translation_56A58(layout_function, )
                layout_function = layout
                ...
                layout_function = layout
                sna_miscellaneous_func_C189F(layout_function, )
...
class SNA_OT_Add_Scatter_System_Fbb77(bpy.types.Operator):
    ...
    def execute(self, context):
        if 'OBJECT'==bpy.context.mode:
            ...
            bpy.ops.wm.call_panel(name="SNA_PT_cant_create_scatter_systems_in_this_mode_773E5", keep_open=False)
        return {"FINISHED"}
...
class SNA_PT_cant_create_scatter_systems_in_this_mode_773E5(bpy.types.Panel):
    bl_label = ''
    bl_idname = 'SNA_PT_cant_create_scatter_systems_in_this_mode_773E5'
    bl_space_type = 'VIEW_3D'
...
class SNA_MT_09C83(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        op = layout.operator('sna.refresh_view_e4454', text='Refresh List', icon_value=string_to_icon('FILE_REFRESH'), emboss=True, depress=False)
    ...
    if bpy.context.scene.sna_display_culling:
        ...
        if bpy.context.scene.sna_display_vertex_group:
            ...
            if (bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].sna_surface_type == 'Emitter Object'):
                row_CE20D.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_102"]', bpy.context.scene.sna_emitter_object, 'vertex_groups', text='', icon='GROUP_VERTEX', item_search_property="name")
            else:
                if (bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].sna_surface_type == 'Single Surface'):
                    row_CE20D.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_102"]', bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_108'], 'vertex_groups', text='', icon='FILE', item_search_property="name")
                else:
                    row_CE20D.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_102"]', bpy.context.view_layer.objects.active, 'vertex_groups', text='', icon='FILE', item_search_property="name")
...
def sna_func_ecosystem_883DD(layout_function, ):
    ...
    if bpy.context.scene.sna_display_ecosystem:
        ...
        if bpy.context.scene.sna_display_attraction:
            ...
            col_F101B.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_162"]', bpy.data.collections['Scatter Systems'], 'objects', text='', icon='COLLAPSEMENU', item_search_property="name")
        ...
        if bpy.context.scene.sna_display_repulsion:
            ...
            col_12930.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_167"]', bpy.data.collections['Scatter Systems'], 'objects', text='', icon='COLLAPSEMENU', item_search_property="name")
...
def sna_func_instances_B4607(layout_function, ):
    ...
    if bpy.context.scene.sna_displayinstances:
        ...
        if (bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].sna_instance_type == 'Multiple Instances'):
            col_BAF6D.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_31"]', bpy.context.scene.collection, 'children', text='', icon='OUTLINER_COLLECTION', item_search_property="name")
        else:
...
def sna_miscellaneous_func_C189F(layout_function, ):
    box_463E9 = layout_function.box()
    box_463E9.alert = False
    box_463E9.enabled = True
    box_463E9.active = True
    box_463E9.use_property_split = False
    box_463E9.use_property_decorate = False
    box_463E9.alignment = 'Expand'.upper()
    box_463E9.scale_x = 1.0
    box_463E9.scale_y = 1.0
    if not True: box_463E9.operator_context = "EXEC_DEFAULT"
    col_4B31D = box_463E9.column(heading='', align=False)
    col_4B31D.alert = False
    col_4B31D.enabled = True
    col_4B31D.active = True
    col_4B31D.use_property_split = False
    col_4B31D.use_property_decorate = False
    col_4B31D.scale_x = 1.0
    col_4B31D.scale_y = 1.0
    col_4B31D.alignment = 'Expand'.upper()
    col_4B31D.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
    row_74903 = col_4B31D.row(heading='', align=False)
    row_74903.alert = False
    row_74903.enabled = True
    row_74903.active = True
    row_74903.use_property_split = False
    row_74903.use_property_decorate = False
    row_74903.scale_x = 1.0
    row_74903.scale_y = 1.0
    row_74903.alignment = 'Left'.upper()
    row_74903.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
    row_74903.prop(bpy.context.scene, 'sna_display_miscellaneous', text='Miscellaneous', icon_value=(string_to_icon('DOWNARROW_HLT') if bpy.context.scene.sna_display_miscellaneous else string_to_icon('RIGHTARROW')), emboss=False)
    if bpy.context.scene.sna_display_miscellaneous:
        col_04E7A = col_4B31D.column(heading='', align=False)
        col_04E7A.alert = False
        col_04E7A.enabled = True
        col_04E7A.active = True
        col_04E7A.use_property_split = False
        col_04E7A.use_property_decorate = False
        col_04E7A.scale_x = 1.0
        col_04E7A.scale_y = 1.0
        col_04E7A.alignment = 'Expand'.upper()
        col_04E7A.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        col_04E7A.separator(factor=1.0)
        row_33933 = col_04E7A.row(heading='', align=False)
        row_33933.alert = False
        row_33933.enabled = True
        row_33933.active = True
        row_33933.use_property_split = False
        row_33933.use_property_decorate = False
        row_33933.scale_x = 1.0
        row_33933.scale_y = 1.0
        row_33933.alignment = 'Expand'.upper()
        row_33933.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        row_33933.prop(bpy.context.scene, 'sna_display_convert_to_mesh', text='', icon_value=(string_to_icon('DOWNARROW_HLT') if bpy.context.scene.sna_display_convert_to_mesh else string_to_icon('RIGHTARROW')), emboss=False)
        row_33933.label(text='Convert to Mesh', icon_value=0)
        if bpy.context.scene.sna_display_convert_to_mesh:
            col_04E7A.separator(factor=1.0)
            row_66906 = col_04E7A.row(heading='', align=False)
            row_66906.alert = False
            row_66906.enabled = True
            row_66906.active = True
            row_66906.use_property_split = False
            row_66906.use_property_decorate = False
            row_66906.scale_x = 1.0
            row_66906.scale_y = 1.0
            row_66906.alignment = 'Expand'.upper()
            row_66906.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            row_66906.label(text='', icon_value=string_to_icon('BLANK1'))
            col_0CF9C = row_66906.column(heading='', align=False)
            col_0CF9C.alert = False
            col_0CF9C.enabled = True
            col_0CF9C.active = True
            col_0CF9C.use_property_split = False
            col_0CF9C.use_property_decorate = False
            col_0CF9C.scale_x = 1.0
            col_0CF9C.scale_y = 1.0
            col_0CF9C.alignment = 'Expand'.upper()
            col_0CF9C.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            row_5F2E2 = col_0CF9C.row(heading='', align=True)
            row_5F2E2.alert = False
            row_5F2E2.enabled = True
            row_5F2E2.active = True
            row_5F2E2.use_property_split = False
            row_5F2E2.use_property_decorate = False
            row_5F2E2.scale_x = 1.0
            row_5F2E2.scale_y = 1.0
            row_5F2E2.alignment = 'Expand'.upper()
            row_5F2E2.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            op = row_5F2E2.operator('sna.convert_to_mesh_op_18726', text='Convert to Mesh', icon_value=0, emboss=True, depress=False)
        col_04E7A.separator(factor=1.0)
        row_B8FAF = col_04E7A.row(heading='', align=False)
        row_B8FAF.alert = False
        row_B8FAF.enabled = True
        row_B8FAF.active = True
        row_B8FAF.use_property_split = False
        row_B8FAF.use_property_decorate = False
        row_B8FAF.scale_x = 1.0
        row_B8FAF.scale_y = 1.0
        row_B8FAF.alignment = 'Expand'.upper()
        row_B8FAF.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        row_B8FAF.prop(bpy.context.scene, 'sna_display_master_seed', text='', icon_value=(string_to_icon('DOWNARROW_HLT') if bpy.context.scene.sna_display_master_seed else string_to_icon('RIGHTARROW')), emboss=False)
        row_B8FAF.label(text='Master Seed', icon_value=0)
        if bpy.context.scene.sna_display_master_seed:
            col_04E7A.separator(factor=1.0)
            row_36E43 = col_04E7A.row(heading='', align=False)
            row_36E43.alert = False
            row_36E43.enabled = True
            row_36E43.active = True
            row_36E43.use_property_split = False
            row_36E43.use_property_decorate = False
            row_36E43.scale_x = 1.0
            row_36E43.scale_y = 1.0
            row_36E43.alignment = 'Expand'.upper()
            row_36E43.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            row_36E43.label(text='', icon_value=string_to_icon('BLANK1'))
            col_3175B = row_36E43.column(heading='', align=False)
            col_3175B.alert = False
            col_3175B.enabled = True
            col_3175B.active = True
            col_3175B.use_property_split = False
            col_3175B.use_property_decorate = False
            col_3175B.scale_x = 1.0
            col_3175B.scale_y = 1.0
            col_3175B.alignment = 'Expand'.upper()
            col_3175B.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            row_B6563 = col_3175B.row(heading='', align=True)
            row_B6563.alert = False
            row_B6563.enabled = True
            row_B6563.active = True
            row_B6563.use_property_split = False
            row_B6563.use_property_decorate = False
            row_B6563.scale_x = 1.0
            row_B6563.scale_y = 1.0
            row_B6563.alignment = 'Expand'.upper()
            row_B6563.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            attr_214A0 = '["' + str('Socket_119' + '"]') 
            row_B6563.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_214A0, text='Master Seed', icon_value=0, emboss=True)
            op = row_B6563.operator('sna.randomize_master_seed_b1c7a', text='', icon_value=_icons['1439410433.png'].icon_id, emboss=True, depress=False)
            op = row_B6563.operator('sna.reset_master_seed_b266e', text='', icon_value=string_to_icon('LOOP_BACK'), emboss=True, depress=False)


class SNA_OT_Convert_To_Mesh_Op_18726(bpy.types.Operator):
    bl_idname = "sna.convert_to_mesh_op_18726"
    bl_label = "Convert to Mesh Op"
    bl_description = "Convert the selected system to a mesh"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        bpy.ops.wm.call_menu(name="SNA_MT_D0112")
        return {"FINISHED"}
...
class SNA_OT_Confirm_Convert_To_Mesh_C046D(bpy.types.Operator):
    bl_idname = "sna.confirm_convert_to_mesh_c046d"
    bl_label = "Confirm Convert to Mesh"
    bl_description = "Confirm and convert to mesh"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        if bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_117']:
            pass
        else:
            bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_117'] = True
            bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].update_tag(refresh={'DATA'}, )
            if bpy.context and bpy.context.screen:
                for a in bpy.context.screen.areas:
                    a.tag_redraw()
        if bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].hide_select:
            bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].hide_select = False
            bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].update_tag(refresh={'DATA'}, )
            if bpy.context and bpy.context.screen:
                for a in bpy.context.screen.areas:
                    a.tag_redraw()
        bpy.context.view_layer.objects.active = bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name]
        bpy.ops.object.modifier_apply('INVOKE_DEFAULT', modifier='GN_SCS_MOD')
        bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].update_tag(refresh={'DATA'}, )
        if bpy.context and bpy.context.screen:
            for a in bpy.context.screen.areas:
                a.tag_redraw()
...
class SNA_MT_D0112(bpy.types.Menu):
    bl_idname = "SNA_MT_D0112"
    bl_label = ""
    ...
    def draw(self, context):
        layout = self.layout.column_flow(columns=1)
        layout.operator_context = "INVOKE_DEFAULT"
        layout.label(text='Are you sure? This action cannot be undone', icon_value=46)
        op = layout.operator('sna.confirm_convert_to_mesh_c046d', text="Yes, I'm sure", icon_value=0, emboss=True, depress=False)
        op = layout.operator('sna.cancel_1ddb4', text='Cancel', icon_value=0, emboss=True, depress=False)


class SNA_OT_Cancel_1Ddb4(bpy.types.Operator):
    bl_idname = "sna.cancel_1ddb4"
    bl_label = "Cancel"
...
class SNA_OT_Reset_Master_Seed_B266E(bpy.types.Operator):
    bl_idname = "sna.reset_master_seed_b266e"
    bl_label = "Reset Master Seed"
    bl_description = "Resets the master seed back to 0"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_119'] = 0
        bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].update_tag(refresh={'DATA'}, )
        if bpy.context and bpy.context.screen:
            for a in bpy.context.screen.areas:
                a.tag_redraw()
...
class SNA_OT_Randomize_Master_Seed_B1C7A(bpy.types.Operator):
    bl_idname = "sna.randomize_master_seed_b1c7a"
    bl_label = "Randomize Master Seed"
    bl_description = "Randomizes the master seed"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_119'] = random_integer(-100000000.0, 100000000.0, None)
        bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].update_tag(refresh={'DATA'}, )
        if bpy.context and bpy.context.screen:
            for a in bpy.context.screen.areas:
                a.tag_redraw()
...
def sna_func_optimization_DB30B(layout_function, ):
    ...
    if bpy.context.scene.sna_display_optimization:
        ...
        row_A4B80 = row_7A000.row(heading='', align=True)
        row_A4B80.alert = False
        row_A4B80.enabled = True
        row_A4B80.active = True
        row_A4B80.use_property_split = False
        row_A4B80.use_property_decorate = False
        row_A4B80.scale_x = 1.0
        row_A4B80.scale_y = 1.0
        row_A4B80.alignment = 'Expand'.upper()
        row_A4B80.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        attr_AF4DA = '["' + str('Socket_125' + '"]') 
        row_A4B80.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_AF4DA, text='Camera Culling', icon_value=0, emboss=True)
        if bpy.context.scene.sna_display_enable_camera_culling:
            if property_exists("bpy.context.scene.camera.data", globals(), locals()):
                ...
                col_82F39.label(text='No Active Camera in scene', icon_value=712)
    ...
    if bpy.context.scene.sna_display_proximity:
        ...
        if bpy.context.scene.sna_display_geometry_proximity:
            ...
            if (bpy.context.scene.sna_proximity_type == 'Single Object'):
                ...
                col_F8767.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_47"]', bpy.data, 'collections', text='Collection', icon='OUTLINER_COLLECTION', item_search_property="name")
            col_F8767.separator(factor=1.0)
        ...
        if bpy.context.scene.sna_display_curve_proximity:
            ...
            if (bpy.context.scene.sna_curve_proximity_type == 'Single Curve'):
                ...
                col_30362.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_60"]', bpy.data, 'collections', text='Collection', icon='OUTLINER_COLLECTION', item_search_property="name")
            col_30362.separator(factor=1.0)
...
class SNA_PT_QUICK_SCATTER_1E6A9(bpy.types.Panel):
    bl_label = 'Quick Scatter'
    bl_idname = 'SNA_PT_QUICK_SCATTER_1E6A9'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_context = ''
    bl_category = 'OpenScatter'
    ...
    def draw(self, context):
        layout = self.layout
        if (bpy.context.scene.sna_emitter_object == None):
            layout.label(text='No Emitter Object Selected', icon_value=string_to_icon('WARNING_LARGE'))
        else:
...
class SNA_OT_Scatter_Viewport_Objects_74948(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if objects_selected_1_602b9:
            ...
            bpy.ops.wm.call_panel(name="SNA_PT_WARNING_49CC5", keep_open=False)
        return {"FINISHED"}
...
def sna_pattern_enum_items(self, context):
    enum_items = [['Normal', 'Normal', '', string_to_icon('LIGHTPROBE_VOLUME')], ['Clumps', 'Clumps', '', string_to_icon('POINTCLOUD_DATA')], ['Lines', 'Lines', '', string_to_icon('ALIGN_JUSTIFY')]]
    return [make_enum_item(item[0], item[1], item[2], item[3], i) for i, item in enumerate(enum_items)]


class SNA_PT_WARNING_49CC5(bpy.types.Panel):
    bl_label = 'Warning!'
    bl_idname = 'SNA_PT_WARNING_49CC5'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'WINDOW'
    bl_context = ''
    bl_order = 0
    ...
    def draw(self, context):
        ...
        col_1C219.label(text='No objects selected!', icon_value=string_to_icon('WARNING_LARGE'))
    ...
    if bpy.context.scene.sna_displaysurfacesample:
        ...
        if bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_179']:
            ...
            if bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_109']:
                col_A4518.prop_search(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], '["Socket_110"]', bpy.context.scene.collection, 'children', text='', icon='OUTLINER_COLLECTION', item_search_property="name")
            else:
...
def sna_func_texturemasks2_E0304(layout_function, ):
    ...
    if bpy.context.scene.sna_display_image:
        ...
        col_EB814 = row_88AA1.column(heading='', align=True)
        ...
        col_55492 = col_EB814.column(heading='', align=False)
        col_55492.alert = False
        col_55492.enabled = True
        col_55492.active = True
        col_55492.use_property_split = False
        col_55492.use_property_decorate = False
        col_55492.scale_x = 1.0
        col_55492.scale_y = 1.0
        col_55492.alignment = 'Expand'.upper()
        col_55492.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        box_3D1E5 = col_55492.box()
        box_3D1E5.alert = False
        box_3D1E5.enabled = True
        box_3D1E5.active = True
        box_3D1E5.use_property_split = False
        box_3D1E5.use_property_decorate = False
        box_3D1E5.alignment = 'Expand'.upper()
        box_3D1E5.scale_x = 1.0
        box_3D1E5.scale_y = 1.0
        if not True: box_3D1E5.operator_context = "EXEC_DEFAULT"
        col_9A788 = box_3D1E5.column(heading='', align=True)
        col_9A788.alert = False
        col_9A788.enabled = True
        col_9A788.active = True
        col_9A788.use_property_split = False
        col_9A788.use_property_decorate = False
        col_9A788.scale_x = 1.0
        col_9A788.scale_y = 1.0
        col_9A788.alignment = 'Expand'.upper()
        col_9A788.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        col_9A788.prop(bpy.context.scene, 'sna_image_mask', text='Image', icon_value=0, emboss=True)
        col_9A788.separator(factor=1.0)
        attr_EEC69 = '["' + str('Socket_182' + '"]') 
        col_9A788.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_EEC69, text='Bounds', icon_value=0, emboss=True)
        col_9A788.separator(factor=1.0)
        attr_875B3 = '["' + str('Socket_183' + '"]') 
        col_9A788.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_875B3, text='Min', icon_value=0, emboss=True)
        attr_7B9A3 = '["' + str('Socket_184' + '"]') 
        col_9A788.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_7B9A3, text='Max', icon_value=0, emboss=True)
        attr_F8EEE = '["' + str('Socket_185' + '"]') 
        col_9A788.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_F8EEE, text='Invert', icon_value=0, emboss=True)
        attr_42A64 = '["' + str('Socket_186' + '"]') 
        col_9A788.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_42A64, text='Sharpen', icon_value=0, emboss=True)
        col_55492.separator(factor=1.0)
        col_17CC7 = col_55492.column(heading='', align=False)
        col_17CC7.alert = False
        col_17CC7.enabled = True
        col_17CC7.active = True
        col_17CC7.use_property_split = False
        col_17CC7.use_property_decorate = False
        col_17CC7.scale_x = 1.0
        col_17CC7.scale_y = 1.0
        col_17CC7.alignment = 'Expand'.upper()
        col_17CC7.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        box_B3CCA = col_17CC7.box()
        box_B3CCA.alert = False
        box_B3CCA.enabled = True
        box_B3CCA.active = True
        box_B3CCA.use_property_split = False
        box_B3CCA.use_property_decorate = False
        box_B3CCA.alignment = 'Expand'.upper()
        box_B3CCA.scale_x = 1.0
        box_B3CCA.scale_y = 1.0
        if not True: box_B3CCA.operator_context = "EXEC_DEFAULT"
        attr_42428 = '["' + str('Socket_194' + '"]') 
        box_B3CCA.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_42428, text='Separate RGB', icon_value=0, emboss=True)
        if bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD']['Socket_194']:
            attr_ADE1C = '["' + str('Socket_195' + '"]') 
            box_B3CCA.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_ADE1C, text='Channel', icon_value=0, emboss=True)
        attr_9B163 = '["' + str('Socket_187' + '"]') 
        col_17CC7.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_9B163, text='Location', icon_value=0, emboss=True)
        attr_3F2B7 = '["' + str('Socket_188' + '"]') 
        col_17CC7.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_3F2B7, text='Rotation', icon_value=0, emboss=True)
        attr_ABD84 = '["' + str('Socket_189' + '"]') 
        col_17CC7.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_ABD84, text='Scale', icon_value=0, emboss=True)


def sna_func_translation_56A58(layout_function, ):
    box_0CD1D = layout_function.box()
    box_0CD1D.alert = False
    box_0CD1D.enabled = True
    box_0CD1D.active = True
    box_0CD1D.use_property_split = False
    box_0CD1D.use_property_decorate = False
    box_0CD1D.alignment = 'Expand'.upper()
    box_0CD1D.scale_x = 1.0
    box_0CD1D.scale_y = 1.0
    if not True: box_0CD1D.operator_context = "EXEC_DEFAULT"
    col_4FF3F = box_0CD1D.column(heading='', align=False)
    col_4FF3F.alert = False
    col_4FF3F.enabled = True
    col_4FF3F.active = True
    col_4FF3F.use_property_split = False
    col_4FF3F.use_property_decorate = False
    col_4FF3F.scale_x = 1.0
    col_4FF3F.scale_y = 1.0
    col_4FF3F.alignment = 'Expand'.upper()
    col_4FF3F.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
    row_1A8BE = col_4FF3F.row(heading='', align=False)
    row_1A8BE.alert = False
    row_1A8BE.enabled = True
    row_1A8BE.active = True
    row_1A8BE.use_property_split = False
    row_1A8BE.use_property_decorate = False
    row_1A8BE.scale_x = 1.0
    row_1A8BE.scale_y = 1.0
    row_1A8BE.alignment = 'Left'.upper()
    row_1A8BE.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
    row_1A8BE.prop(bpy.context.scene, 'sna_display_translation_panel', text='Translation', icon_value=(string_to_icon('DOWNARROW_HLT') if bpy.context.scene.sna_display_translation_panel else string_to_icon('RIGHTARROW')), emboss=False)
    if bpy.context.scene.sna_display_translation_panel:
        col_B8444 = col_4FF3F.column(heading='', align=False)
        col_B8444.alert = False
        col_B8444.enabled = True
        col_B8444.active = True
        col_B8444.use_property_split = False
        col_B8444.use_property_decorate = False
        col_B8444.scale_x = 1.0
        col_B8444.scale_y = 1.0
        col_B8444.alignment = 'Expand'.upper()
        col_B8444.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        col_84057 = col_B8444.column(heading='', align=False)
        col_84057.alert = False
        col_84057.enabled = True
        col_84057.active = True
        col_84057.use_property_split = False
        col_84057.use_property_decorate = False
        col_84057.scale_x = 1.0
        col_84057.scale_y = 1.0
        col_84057.alignment = 'Expand'.upper()
        col_84057.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        col_84057.separator(factor=1.0)
        row_57130 = col_84057.row(heading='', align=True)
        row_57130.alert = False
        row_57130.enabled = True
        row_57130.active = True
        row_57130.use_property_split = False
        row_57130.use_property_decorate = False
        row_57130.scale_x = 1.0
        row_57130.scale_y = 1.0
        row_57130.alignment = 'Expand'.upper()
        row_57130.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
        row_57130.prop(bpy.context.scene, 'sna_display_rotate', text='', icon_value=(string_to_icon('DOWNARROW_HLT') if bpy.context.scene.sna_display_rotate else string_to_icon('RIGHTARROW')), emboss=False)
        attr_BD9C3 = '["' + str('Socket_201' + '"]') 
        row_57130.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_BD9C3, text='Translation', icon_value=0, emboss=True)
        if bpy.context.scene.sna_display_rotate:
            row_2EEA2 = col_84057.row(heading='', align=False)
            row_2EEA2.alert = False
            row_2EEA2.enabled = True
            row_2EEA2.active = True
            row_2EEA2.use_property_split = False
            row_2EEA2.use_property_decorate = False
            row_2EEA2.scale_x = 1.0
            row_2EEA2.scale_y = 1.0
            row_2EEA2.alignment = 'Expand'.upper()
            row_2EEA2.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            row_2EEA2.label(text='', icon_value=string_to_icon('BLANK1'))
            col_B267B = row_2EEA2.column(heading='', align=False)
            col_B267B.alert = False
            col_B267B.enabled = True
            col_B267B.active = True
            col_B267B.use_property_split = False
            col_B267B.use_property_decorate = False
            col_B267B.scale_x = 1.0
            col_B267B.scale_y = 1.0
            col_B267B.alignment = 'Expand'.upper()
            col_B267B.operator_context = "INVOKE_DEFAULT" if True else "EXEC_DEFAULT"
            col_B267B.separator(factor=1.0)
            col_B267B.label(text='Translation:', icon_value=string_to_icon('DRIVER_TRANSFORM'))
            attr_9426A = '["' + str('Socket_198' + '"]') 
            col_B267B.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_9426A, text='', icon_value=0, emboss=True)
...
class SNA_OT_Update_8D639(bpy.types.Operator):
    ...
    def execute(self, context):
        version_comparison_0_05fa3 = sna_get_latest_release_func_BC0A1((1, 0, 7))
        if (version_comparison_0_05fa3 == 'higher'):
            bpy.ops.wm.call_panel(name="SNA_PT_USING_NEWER_VERSION_2A1C9", keep_open=False)
        else:
            if (version_comparison_0_05fa3 == 'lower'):
                bpy.ops.wm.call_panel(name="SNA_PT_NEW_VERSION_AVAILABLE_FF6D7", keep_open=False)
            else:
                bpy.ops.wm.call_panel(name="SNA_PT_NO_NEW_VERSION_AVAILABLE_E1BFB", keep_open=False)
        return {"FINISHED"}
...
class SNA_PT_NEW_VERSION_AVAILABLE_FF6D7(bpy.types.Panel):
    bl_label = 'New Version Available'
    bl_idname = 'SNA_PT_NEW_VERSION_AVAILABLE_FF6D7'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'WINDOW'
    bl_context = ''
    bl_order = 0
...
class SNA_PT_NO_NEW_VERSION_AVAILABLE_E1BFB(bpy.types.Panel):
    bl_label = 'No New Version Available'
    bl_idname = 'SNA_PT_NO_NEW_VERSION_AVAILABLE_E1BFB'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'WINDOW'
    bl_context = ''
    bl_order = 0
...
class SNA_PT_USING_NEWER_VERSION_2A1C9(bpy.types.Panel):
    bl_label = 'Using Newer Version'
    bl_idname = 'SNA_PT_USING_NEWER_VERSION_2A1C9'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'WINDOW'
    bl_context = ''
    bl_order = 0
        ...
        if not (False):
            ...
            if bpy.context.scene.sna_display_preferences_settings:
                ...
                if bpy.context.scene.sna_display_auto_scene_switch_detection:
                    ...
                    op = row_862C6.operator('sna.documentation_7e9c9', text='', icon_value=string_to_icon('QUESTION'), emboss=False, depress=False)
...
def register():
    ...
    bpy.types.Scene.sna_display_translation_panel = bpy.props.BoolProperty(name='Display Translation Panel', description='', default=False)
    bpy.types.Scene.sna_display_translation = bpy.props.BoolProperty(name='Display Translation', description='', default=False)
    bpy.types.Scene.sna_display_random_translation = bpy.props.BoolProperty(name='Display Random Translation', description='', default=False)
    bpy.types.Scene.sna_display_normal_offset = bpy.props.BoolProperty(name='Display Normal Offset', description='', default=False)
    bpy.types.Scene.sna_display_random_normal_offset = bpy.props.BoolProperty(name='Display Random Normal Offset', description='', default=False)
    bpy.types.Scene.sna_display_miscellaneous = bpy.props.BoolProperty(name='Display Miscellaneous', description='', default=False)
    bpy.types.Scene.sna_display_convert_to_mesh = bpy.props.BoolProperty(name='Display Convert to Mesh', description='', default=False)
    bpy.types.Scene.sna_display_master_seed = bpy.props.BoolProperty(name='Display Master Seed', description='', default=False)
    bpy.utils.register_class(SNA_PT_OPENSCATTER_85C9A)
    ...
    bpy.utils.register_class(SNA_PT_cant_create_scatter_systems_in_this_mode_773E5)
    ...
    bpy.utils.register_class(SNA_OT_Convert_To_Mesh_Op_18726)
    bpy.utils.register_class(SNA_OT_Confirm_Convert_To_Mesh_C046D)
    bpy.utils.register_class(SNA_MT_D0112)
    bpy.utils.register_class(SNA_OT_Cancel_1Ddb4)
    bpy.utils.register_class(SNA_OT_Reset_Master_Seed_B266E)
    bpy.utils.register_class(SNA_OT_Randomize_Master_Seed_B1C7A)
    bpy.utils.register_class(SNA_AddonPreferences_14B35)
    bpy.utils.register_class(SNA_OT_Documentation_7E9C9)
    bpy.utils.register_class(SNA_PT_QUICK_SCATTER_1E6A9)
    bpy.utils.register_class(SNA_OT_Scatter_Viewport_Objects_74948)
    bpy.utils.register_class(SNA_PT_WARNING_49CC5)
    if not '1439410433.png' in _icons: _icons.load('1439410433.png', os.path.join(os.path.dirname(__file__), 'icons', '1439410433.png'), "IMAGE")
    ...
    bpy.utils.register_class(SNA_PT_NEW_VERSION_AVAILABLE_FF6D7)
    bpy.utils.register_class(SNA_PT_NO_NEW_VERSION_AVAILABLE_E1BFB)
    bpy.utils.register_class(SNA_PT_USING_NEWER_VERSION_2A1C9)
...
def unregister():
    ...
    addon_keymaps.clear()
    del bpy.types.Scene.sna_display_master_seed
    del bpy.types.Scene.sna_display_convert_to_mesh
    del bpy.types.Scene.sna_display_miscellaneous
    del bpy.types.Scene.sna_display_random_normal_offset
    del bpy.types.Scene.sna_display_normal_offset
    del bpy.types.Scene.sna_display_random_translation
    del bpy.types.Scene.sna_display_translation
    del bpy.types.Scene.sna_display_translation_panel
    ...
    bpy.utils.unregister_class(SNA_PT_OPENSCATTER_85C9A)
    ...
    bpy.utils.unregister_class(SNA_PT_cant_create_scatter_systems_in_this_mode_773E5)
    bpy.utils.unregister_class(SNA_MT_09C83)
    bpy.utils.unregister_class(SNA_OT_Enter_Weight_Paint_Mode_Db6F9)
    bpy.utils.unregister_class(SNA_OT_Convert_To_Mesh_Op_18726)
    bpy.utils.unregister_class(SNA_OT_Confirm_Convert_To_Mesh_C046D)
    bpy.utils.unregister_class(SNA_MT_D0112)
    bpy.utils.unregister_class(SNA_OT_Cancel_1Ddb4)
    bpy.utils.unregister_class(SNA_OT_Reset_Master_Seed_B266E)
    bpy.utils.unregister_class(SNA_OT_Randomize_Master_Seed_B1C7A)
    bpy.utils.unregister_class(SNA_AddonPreferences_14B35)
    bpy.utils.unregister_class(SNA_OT_Documentation_7E9C9)
    bpy.utils.unregister_class(SNA_PT_QUICK_SCATTER_1E6A9)
    bpy.utils.unregister_class(SNA_OT_Scatter_Viewport_Objects_74948)
    bpy.utils.unregister_class(SNA_PT_WARNING_49CC5)
    ...
    bpy.utils.unregister_class(SNA_PT_NEW_VERSION_AVAILABLE_FF6D7)
    bpy.utils.unregister_class(SNA_PT_NO_NEW_VERSION_AVAILABLE_E1BFB)
    bpy.utils.unregister_class(SNA_PT_USING_NEWER_VERSION_2A1C9)
