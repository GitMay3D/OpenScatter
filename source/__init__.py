...
class SNA_UL_display_collection_list_29FCA(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, data, item_29FCA, icon, active_data, active_propname, index_29FCA):
        ...
        row_F750B = layout.row(heading='', align=True)
...
class SNA_PT_OPENSCATTER_38B00(bpy.types.Panel):
    bl_label = 'OpenScatter'
    bl_idname = 'SNA_PT_OPENSCATTER_38B00'
    ...
    def draw(self, context):
        ...
        if (bpy.context.scene.sna_emitter_object == None):
            ...
            if bpy.context.scene.sna_display_system_list:
                ...
                col_8104B.separator(factor=1.0)
                op = col_8104B.operator('sna.refresh_view_e4454', text='', icon_value=647, emboss=True, depress=False)
        if (bpy.context.scene.sna_emitter_object == None):
            pass
        else:
...
class SNA_OT_Add_Scatter_System_Fbb77(bpy.types.Operator):
    ...
    def execute(self, context):
        if 'OBJECT'==bpy.context.mode:
            ...
            bpy.ops.wm.call_panel(name="SNA_PT_cant_create_scatter_systems_in_this_mode_4B255", keep_open=False)
        return {"FINISHED"}
...
class SNA_PT_cant_create_scatter_systems_in_this_mode_4B255(bpy.types.Panel):
    bl_label = ''
    bl_idname = 'SNA_PT_cant_create_scatter_systems_in_this_mode_4B255'
    bl_space_type = 'VIEW_3D'
...
def sna_func_culling_CEC85(layout_function, ):
    ...
    if bpy.context.scene.sna_display_culling:
        ...
        if bpy.context.scene.sna_display_vertex_group:
            ...
            row_5E09E.prop(bpy.data.objects[bpy.context.scene.sna_scattersystemcollection[bpy.context.scene.sna_active_index].name].modifiers['GN_SCS_MOD'], attr_77955, text='', icon_value=string_to_icon('ARROW_LEFTRIGHT'), emboss=True)
            op = row_5E09E.operator('sna.enter_weight_paint_mode_db6f9', text='', icon_value=string_to_icon('WPAINT_HLT'), emboss=True, depress=False)
            col_ED557.separator(factor=1.0)
            col_ED557.label(text='Falloff:', icon_value=string_to_icon('SHARPCURVE'))
...
class SNA_AddonPreferences_14B35(bpy.types.AddonPreferences):
    ...
    sna_auto_scene_switch_detection: bpy.props.BoolProperty(name='Auto Scene Switch Detection', description='', default=False)
    ...
    def draw(self, context):
        if not (False):
            ...
            box_B7037.prop(bpy.context.scene, 'sna_display_preferences_settings', text='Settings', icon_value=(string_to_icon('TRIA_DOWN') if bpy.context.scene.sna_display_preferences_settings else string_to_icon('TRIA_RIGHT')), emboss=False)
            if bpy.context.scene.sna_display_preferences_settings:
                box_C65D7 = box_B7037.box()
                ...
                if bpy.context.scene.sna_display_auto_scene_switch_detection:
                    ...
                    col_9E438.label(text='This feature is highly experimental and not recommended unless you switch between scenes very often.', icon_value=0)
                    col_9E438.label(text="If you're not sure what it does, please read the documentation.", icon_value=0)
...
class SNA_PT_QUICK_SCATTER_07639(bpy.types.Panel):
    bl_label = 'Quick Scatter'
    bl_idname = 'SNA_PT_QUICK_SCATTER_07639'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_context = ''
    bl_category = 'OpenScatter'
...
class SNA_OT_Scatter_Viewport_Objects_74948(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if objects_selected_1_602b9:
            ...
            bpy.context.scene.sna_instance_type = 'Multiple Instances'
            ...
            bpy.ops.wm.call_panel(name="SNA_PT_WARNING_4EA34", keep_open=False)
        return {"FINISHED"}
...
class SNA_PT_WARNING_4EA34(bpy.types.Panel):
    bl_label = 'Warning!'
    bl_idname = 'SNA_PT_WARNING_4EA34'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'WINDOW'
    bl_context = ''
    bl_order = 0
...
@persistent
def frame_change_post_handler_8DBB4(dummy):
...
def register():
    ...
    bpy.utils.register_class(SNA_PT_OPENSCATTER_38B00)
    ...
    bpy.utils.register_class(SNA_PT_cant_create_scatter_systems_in_this_mode_4B255)
    ...
    bpy.utils.register_class(SNA_PT_QUICK_SCATTER_07639)
    bpy.utils.register_class(SNA_OT_Scatter_Viewport_Objects_74948)
    bpy.utils.register_class(SNA_PT_WARNING_4EA34)
    ...
    bpy.app.handlers.frame_change_post.append(frame_change_post_handler_8DBB4)
...
def unregister():
    ...
    bpy.utils.unregister_class(SNA_PT_OPENSCATTER_38B00)
    ...
    bpy.utils.unregister_class(SNA_PT_cant_create_scatter_systems_in_this_mode_4B255)
    ...
    bpy.utils.unregister_class(SNA_PT_QUICK_SCATTER_07639)
    bpy.utils.unregister_class(SNA_OT_Scatter_Viewport_Objects_74948)
    bpy.utils.unregister_class(SNA_PT_WARNING_4EA34)
    ...
    bpy.app.handlers.frame_change_post.remove(frame_change_post_handler_8DBB4)
